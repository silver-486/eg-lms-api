...
public class GetAssignmentByUserIdCourseIdHandler extends CommandHandler {
	...
	public void handleCommand(CommandWrapper commandWrapper) throws JsonProcessingException {
		...
		if (Command.GET_ASSIGNMENT_BY_USER_ID_COURSE_ID != command) {
			if (next == null) {
				return;
			}
			...
			commandWrapper.addPayload(mocker.buildPayload(command, mocker.randomId(), mocker.mockIds(2)));
		}
	}
}
