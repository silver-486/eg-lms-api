...
public class UpdatePicsAccountCommandHandler extends CommandHandler {
	...
	public void handleCommand(CommandWrapper commandWrapper) throws JsonProcessingException {
		...
		if (Command.UPDATE_PICS_ACCOUNT != command) {
			if (next == null) {
				return;
			}
			...
			commandWrapper.addPayload(mocker.buildPayload(command, mocker.randomId(), mocker.mockPicsAccount()));
		}
	}
}
