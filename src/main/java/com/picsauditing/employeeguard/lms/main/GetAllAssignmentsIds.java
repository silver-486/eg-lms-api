...
public class GetAllAssignmentsIds extends CommandHandler {
	...
	public void handleCommand(CommandWrapper commandWrapper) throws JsonProcessingException {
		...
		if (Command.GET_ALL_ASSIGNMENTS_IDS != command) {
			if (next == null) {
				return;
			}
			...
			commandWrapper.addPayload(mocker.buildPayload(command, mocker.randomId(), null));
		}
	}
}
